...
::mlir::DenseElementsAttr CreateDenseAttrFromLiteral(
  ...
  if constexpr (std::is_same<CppType, u4>::value ||
                std::is_same<CppType, s4>::value) {
...
void CopyDenseElementsBy<u4>(mlir::DenseElementsAttr data,
                             std::vector<uint8_t>* output) {
  output->resize((data.getNumElements() + 1) / 2);
  PackInt4(data.getRawData().data(), reinterpret_cast<char*>(output->data()),
           data.getNumElements());
}
