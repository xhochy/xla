...
// Represents a P2P group node and the computation that contains the P2P
// instructions. An unpipelined P2P group contains only one node while a
// pipelined P2P group contains a node for the while-body and a node for the
// computation with the while-loop instruction calling the while-body.
struct P2PGroupNode {
  ...
  bool RecordDoneOp(HloSendRecvInstruction* p2p) {
    ...
    if (p2p->opcode() == HloOpcode::kRecvDone) {
      if (recv_done == nullptr) {
        recv_done = DynCast<HloRecvDoneInstruction>(p2p);
        return true;
      }
    } else if (p2p->opcode() == HloOpcode::kSendDone) {
      if (send_done == nullptr) {
        send_done = DynCast<HloSendDoneInstruction>(p2p);
        return true;
      }
    }
    return false;
...
// operations. We use std::vector instead of hash set for deterministic
// iterators.
using P2PInComputation =
    absl::flat_hash_map<const HloComputation*, std::vector<int64_t>>;
void RecordChannel(std::vector<int64_t>& channels, int64_t channel) {
...
Status CollectP2PGroups(const HloComputation* computation,
  ...
  for (auto it = all_instructions.begin(); it != all_instructions.end(); ++it) {
    ...
    HloSendRecvInstruction* p2p = DynCast<HloSendRecvInstruction>(hlo);
    ...
    if (p2p_in_comp == p2p_in_computation.end()) {
      ...
      // Add the channel only if it is not recorded yet, assuming there are
      // at most a few P2P channel in a computation.
      RecordChannel(p2p_in_comp->second, channel);
    }
  }
...
StatusOr<bool> P2PSchedulePreparation::Run(
  ...
  for (auto iter = all_computations.rbegin(); iter != all_computations.rend();
    ...
    std::vector<int64_t>& p2p_channels = p2p_in_comp->second;
    ...
    for (auto it = all_instructions.begin(); it != all_instructions.end();
      ...
      VLOG(0) << "P2P result: " << p2p_result->first->ToString();
