...
// pair of asynchronous collective operation. This includes Send and Recv
// operations.
bool IsCollectiveOp(const HloInstruction* instruction) {
...
Status ChainUnpipelinedP2P(const P2PGroupNode& node) {
...
Status ChainPipelinedP2PChild(const P2PGroupNode& node) {
  HloSendRecvInstruction* recv_done = node.recv_done;
  HloRecvInstruction* recv =
      DynCast<HloRecvInstruction>(recv_done->mutable_operand(0));
  HloSendRecvInstruction* send_done = node.send_done;
...
Status ChainCollectivesWithUnpipelinedP2P(
  ...
  // Recv-Done, add control dependence to make sure it is scheduled before
  // Send.
  for (auto it = begin; it != recv_done_iter; ++it) {
    HloInstruction* hlo = *it;
    if (!MayInvokeCollectiveOp(hlo, collective_in_computation) ||
        in_current_p2p_chain(hlo)) {
      continue;
    }
    if (!reachability->IsReachable(hlo, send)) {
      TF_RETURN_IF_ERROR(hlo->AddControlDependencyTo(send));
      changed = true;
    }
  }

  // If an hlo may invoke collective operation and is ordered after the
  // Recv-Done, add control dependence to make sure it is scheduled after
  // Send-Done.
  for (auto it = recv_done_iter + 1; it != end; ++it) {
    HloInstruction* hlo = *it;
    if (!MayInvokeCollectiveOp(hlo, collective_in_computation)) {
      continue;
    }
    if (!reachability->IsReachable(send_done, hlo)) {
      TF_RETURN_IF_ERROR(send_done->AddControlDependencyTo(hlo));
      changed = true;
    }
  }

  if (changed) {
    // After we add control dependence, release reachability so that it will
    // be recalculated.
...
Status ChainCollectivesWithPipelinedP2PParent(
  ...
  // If an hlo may invoke collective operation and is ordered after the
  // while-loop, add control dependence to make sure it is scheduled before
  // Recv.
  for (auto it = begin; it != while_loop_iter; ++it) {
    HloInstruction* hlo = *it;
    if (!MayInvokeCollectiveOp(hlo, collective_in_computation) ||
        in_current_p2p_chain(hlo)) {
      continue;
    }
    if (!reachability->IsReachable(hlo, recv)) {
      TF_RETURN_IF_ERROR(hlo->AddControlDependencyTo(recv));
      changed = true;
    }
  }

  // If an hlo may invoke collective operation and is ordered after the
  // while-loop, add control dependence to make sure it is scheduled after
  // Send-Done.
  for (auto it = while_loop_iter + 1; it != end; ++it) {
    HloInstruction* hlo = *it;
    if (!MayInvokeCollectiveOp(hlo, collective_in_computation)) {
      continue;
    }
    ...
    for (auto instr_it = begin; instr_it != end; ++instr_it) {
      ...
      if (kind == P2PGroupKind::kUnpipelined &&
        ...
        TF_RETURN_IF_ERROR(ChainCollectivesWithUnpipelinedP2P(
            p2p_group.nodes[kUnpipelinedNodeIdx], begin, instr_it, end,
            collective_in_computation, reachability));
        ...
        TF_RETURN_IF_ERROR(ChainCollectivesWithPipelinedP2PParent(
            p2p_group.nodes[kPipelinedParentNodeIdx], begin, while_loop_it, end,
            collective_in_computation, reachability));
      } else if (kind == P2PGroupKind::kPipelined &&
                 computation !=
                     p2p_group.nodes[kPipelinedParentNodeIdx].computation &&
                 hlo->opcode() == HloOpcode::kSend) {
        // Case 3: Pipelined P2P chain in the "child", that is, the comptation
        // for the while-body:
        //   Send => Send-Done. ... Recv => Recv-Done
        // All collective ops should be scheduled after Send-Done and Before
        // Recv.
        TF_RETURN_IF_ERROR(ChainCollectivesWithPipelinedP2PChild(
            p2p_group.nodes[kPipelinedChildNodeIdx], begin, end,
            collective_in_computation, reachability));
      }
    }
  }
