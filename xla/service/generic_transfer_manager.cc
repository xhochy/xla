...
Status TransferInt4ArrayFromDevice(se::Stream* stream,
  ...
  stream->ThenDoHostCallback([destination, num_elements,
    ...
    UnpackInt4(moved_dst_data->data(), static_cast<char*>(destination),
               num_elements);
  });
  return OkStatus();
}
...
Status TransferInt4ArrayToDevice(se::Stream* stream, int64_t num_elements,
                                 const void* source,
                                 se::DeviceMemoryBase* destination) {
  auto packed_src_data =
      std::make_unique<std::vector<char>>((num_elements + 1) / 2);
  PackInt4(static_cast<const char*>(source), packed_src_data->data(),
           num_elements);
