...
struct PJRT_TopologyDescription {
  // nullptr iff the PjRtTopologyDescription isn't owned by the caller.
...
struct PJRT_Client {
  ...
  PJRT_Client(std::unique_ptr<xla::PjRtClient> cpp_client);
};
...
// destroyed with PJRT_TopologyDescription_Destroy.
PJRT_TopologyDescription* CreateWrapperDeviceTopology(
